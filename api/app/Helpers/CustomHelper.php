...
class CustomHelper{
    ...
    public static function getContentCount($course_id,$slug){
       $type ='';
        if($slug == 'test-series'){
            $type = 'exam';
        }
        ...
        if($slug == 'live-class'){
            $live_class = LiveClass::where('course_id', $course_id)->where('status', 1)->where('is_delete', 0);

            $count = $live_class->count();
        }else{
...
public static function getImageUrl($path,$filename){
 ...
 if(!empty($filename) ){
    ...
    return url('/public/web_assets/image/ambitiousbaba%201.png');
}
}
