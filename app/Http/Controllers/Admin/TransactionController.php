...
Class TransactionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // if(!empty($search)){
        //     $transactions->where('txn_no','like', '%' . $search . '%');
        // }

        // $transactions = $transactions->paginate(10);


        $transactions = Transaction::select('transactions.*','users.name','users.email','users.mobile_number')
        ->leftJoin('users', 'users.id', '=', 'transactions.user_id');
         if(!empty($search)){
            $transactions->where('transactions.txn_no','like', '%' . $search . '%');
            $transactions->orWhere('users.name','like', '%' . $search . '%');
            $transactions->orWhere('users.email','like', '%' . $search . '%');
            $transactions->orWhere('users.mobile_number','like', '%' . $search . '%');
        }

        $transactions = $transactions->orderBy('transactions.created_at','DESC')->paginate(10);
