...
Class LoanRequestController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['search'] = $search;
        $loan_details = LoanDetails::orderBy('id','desc');
        if(!empty($search)){
            $loan_details->where('name', 'like', '%' . $search . '%');
            $loan_details->orWhere('email', 'like', '%' . $search . '%');
            $loan_details->orWhere('phone', 'like', '%' . $search . '%');
        }
        $loan_details = $loan_details->paginate(10);
        $data['loan_details'] = $loan_details;
        return view('admin.loan_details.index',$data);
    }
...
public function loan_details(Request $request){
    ...
    if(!empty($loan_details)){
        $users = User::where('id',$loan_details->user_id)->first();
    }


    $user_documents = UserDocument::where('user_id',$users->id)->where('loan_id',$id)->first();
    $bank_details = BankDetails::where('user_id',$users->id)->where('loan_id',$id)->first();
    ...
    return view('admin.loan_details.profile',$data);
}
